...
| 场景 | 必读文档 | 只在需要时读取 |
|------|----------|----------------|
| 开始任何 MVP 开发 | `dev/docs/MVP总体规划.md` | `dev/docs/项目管理文档.md` |
| 需要理解文件字段和输入输出 | `dev/docs/MVP关键工件契约.md` | `dev/docs/数据库设计.md` |
| 做小龙虾 1 打标 | `newsletter-tagging/SKILL.md`、`newsletter-tagging/references/workflow.md`、`newsletter-tagging/references/llm_prompt.md` | `newsletter-tagging/schemas/tag_output.schema.json`、`newsletter-tagging/scripts/validate_tags.py` |
| 改标签口径 | `dev/docs/标签字段字典.md` | `newsletter-tagging/references/标签字段字典.md` |
| 做 HTML 渲染 | `dev/docs/MVP关键工件契约.md` 的 `report_content.json` 部分 | `reference/禾大newsletter/` prototype |
| 做数据库兼容 | `dev/docs/数据库设计.md` | `dev/docs/系统结构.md` |

关键工件契约不放在本文档中。需要字段结构、生成者、消费者、校验规则时，读取 `dev/docs/MVP关键工件契约.md`。

## 二、MVP 总原则

1. 当前主线是月度 JSON 工作流，不是数据库实现。
2. 所有运行工件放在 `outputs/<month>/`。
...
- 实现 `app/scripts/clean_rss.py --month <month>`，生成 `rss_clean.json`。
- 实现或完善 `tagging.json` validator。
- 实现 `app/scripts/build_report_context.py --month <month> --lookback 3`。
- 定义并校验 `report_content.json`。
- 实现 `app/scripts/render_report.py --month <month>`。
- 实现端到端控制脚本，写入 `run_log.json`。

开发要求：

- 先检查文件和目录是否存在，不要猜路径。
- 脚本通过 `--month` 参数定位 `outputs/<month>/`。
- 每一步都要明确输入、输出和失败处理。
- 对 JSON 输出做格式化，方便人工阅读。
- 写入新工件前尽量保留运行日志，避免覆盖后无法追溯。
- 修改范围保持在 `app/`、`dev/docs/`、`dev/scripts/`、`newsletter-tagging/` 等相关路径。
...
- `tagging.json` 能通过 `newsletter-tagging/scripts/validate_tags.py`。
...
- `outputs/<month>/rss_clean.json`
- `outputs/<month>/tagging.json`
- `outputs/<month>/report_context.json`
- 近 3 个月最终报告摘要

输出：

- `outputs/<month>/report_content.json`
...
每一步都应记录到 `outputs/<month>/run_log.json`：
...
3. 是否引用了 `dev/docs/MVP关键工件契约.md`，而不是临时发明字段。
4. 是否运行了能运行的 schema、语法或渲染检查。
5. 是否把发现的问题写入 `dev/docs/问题跟踪.md`。
