...
> 本文档定义 MVP 月度工作包中的关键文件契约。`dev/docs/MVP总体规划.md` 只描述项目计划与团队协作；具体字段、生成者、消费者、校验规则以本文档为准。
...
`<month>` 当前采用短月份编号，例如 `2606`。所有脚本和 Agent 必须通过月份参数定位文件，不应写死 `2606`。
...
- 字段应与 `dev/docs/标签字段字典.md` 的基础数据字段保持一致。
