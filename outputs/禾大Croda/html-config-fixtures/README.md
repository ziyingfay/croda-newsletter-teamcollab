...
- 按月生成的静态 HTML 月报：`outputs/<month>/report.html`。
...
这些配置来自 `reference/禾大newsletter/link_exports/` 中的链接导出文件。它们是开发和测试 fixture，不代表最终报告正文质量；真实洞察与正文应由小龙虾 2 基于 `rss_clean.json`、`tagging.json` 和 `report_context.json` 生成。
