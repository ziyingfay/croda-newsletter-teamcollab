...
1. MVP code reads candidate articles from the fixed handoff file `product/output/待打标.json`; that file must be an exact duplicate of the traceable archive at `product/output/result/spider/<YYYYMM>/<timestamp>-rss-clean.json`.
...
5. Code runs schema validation, then `python3 product/skills/newsletter-tagging/scripts/validate_tags.py output.json`.
6. Code runs deterministic post-processing rules from `references/workflow.md`.
7. MVP code writes validated results to `product/output/result/tagging/<YYYYMM>/tagging.json`.
