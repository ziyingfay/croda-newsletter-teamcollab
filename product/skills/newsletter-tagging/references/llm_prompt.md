...
You do not control workflow, retries, status transitions, database writes, sampling, or validation. Code handles those. Your only job is to read one article and return strict JSON that follows `product/skills/newsletter-tagging/schemas/tag_output.schema.json`.
...
Base fields (`source`, `source_name`, `source_list`, `content_language`, `market_region`, etc.) are written by the clean/ingest script onto the article record before tagging. Do not recompute or output them; emit only the Agent judgment tags below.
...
- Article fields (already populated by the clean/ingest script): `article_id`, `title`, `summary`, `content`, `content_length`, `extraction_status`, `source_name`, `source_key`, `source`, `source_list`, `url`, `raw_tags`, `content_language`, `market_region`.
- Source metadata.
...
1. Decide `relevance`: `relevant`, `not_relevant`, or `unclear`.
   - `relevant`: the article involves beauty/personal-care ingredients, formulation, functional claims, technology innovation, research, or regulation and has value for Croda market intelligence. Downstream brand news is relevant only when it involves ingredients, formulation, raw-material procurement, or functional claims.
   - `not_relevant`: pure celebrity endorsement, pure channel/sales data without ingredient relevance, pure internal administrative personnel changes, or pure consumer promotion.
   - `unclear`: the available article text is insufficient to judge the Croda Beauty ingredient-intelligence relevance.
...
7. For the open fields (`ingredient_technology`, `product_application`, `functional_claim`), use controlled labels when possible. If the article clearly mentions a concrete value absent from the dictionary, write an inline label like `other:exosome_lipid_nanoparticle`, add `extracted_name` in the evidence record, and keep the article `tagged`. Prefer over-extracting a new value to missing a new trend. Normalize `other:` slugs to lowercase snake_case: convert spaces and hyphens to underscores, remove leading articles (`a`, `an`, `the`), and singularize simple trailing plurals.
8. Extract `company` freely when the company is the subject of the news event, the article is centered on the company, or the company is an important partner in the event. Do not add a company that merely provides a quoted employee, appears in navigation/ads/related links, appears in a long incidental list, or is only cited as a passing example such as "another brand also uses this ingredient." Do not judge company role; the backend derives it from the watchlist.
