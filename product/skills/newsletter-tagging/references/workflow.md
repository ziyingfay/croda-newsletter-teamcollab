...
- MVP: read pending articles from `product/output/待打标.json`, which must match the traceable archive at `product/output/result/spider/<YYYYMM>/<timestamp>-rss-clean.json`.
...
- MVP: write validated tagging results to `product/output/result/tagging/<YYYYMM>/tagging.json`.
...
```text
product/output/result/spider/<YYYYMM>/<timestamp>-rss-clean.json
...
→ product/output/result/<component>/<YYYYMM>/run_log.json
```
...
- `--month 202606`: monthly public result folder under `product/output/result/`.
- `--input product/output/result/spider/202606/<timestamp>-rss-clean.json`: explicit JSON input path.
- `--output product/output/result/tagging/202606/tagging.json`: explicit JSON output path.
